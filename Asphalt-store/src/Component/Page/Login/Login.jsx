...
const Login = () => {
 ...
 const from = location.state?.from?.pathname || "/";
  ...
  useEffect(() => {
    document.title = "Asphalt |  Login"; // Set the new title here
  }, []);
  
  const handlloginWithGoogle = () => {
    loinWithGoogle()
      .then((result) => {
        toast.success("Login success ,happy shopping"); 
        const user = result.user;
  ...
  return (
    <div>
      ...
      <h1 className="text-center mt-12 font-bold text-2xl">Login Now</h1>
      <form className="beautiful-form" onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <p className="text-sm text-red-600">Forget Password ?</p>
        </div>
        <button type="submit">Submit</button>
        <NavLink to={"/signup"}>
          {" "}
          <p className="text-blue-500 my-4 font-serif text-sm cursor-pointer">
            Create new accout <span>Sign Up</span>
          </p>
        </NavLink>
        <hr className="mt-5" />
        <h1 className=" uppercase text-center text-blue-400 font-semibold">
          Or
        </h1>

        <img
          onClick={handlloginWithGoogle}
          className="w-8 h-8 google "
